<?php
    ...
    // Fetch deposits
    $stmt = $conn->prepare("SELECT id, description, date, category_id, category, amount FROM deposit WHERE user_id = ?");
    if (!$stmt) {
    ...
    $stmt->close();
    createLog($conn, $user_id, "Fetched deposits for deposit table successfully for $username.", 1);
    ...
    $stmt_dates->close();
    createLog($conn, $user_id, "Fetched distinct years for deposit filters successfully for $username.", 1);
